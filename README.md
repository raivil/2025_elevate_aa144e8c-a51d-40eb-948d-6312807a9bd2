...
* Background jobs are used to update subscription statuses periodically, either in bulk, or when the value is older than 24 hours.
...
* A job scheduler was not included.
* An enqueuer job was created to allow bulk updating of user statuses. This job could be enqueue to run daily.
An alternative is to the billing system send a notice to the api with the updated status, once it changes.

* Proper logging was not included on this application.
* There's an issue with configuration of devise-jwt, and a quick fix was added to unblock development.
* There are no permission configuration on this app, only authentication. It could be implemented with ActionPolicy, Pundit, cancancan, etc.
